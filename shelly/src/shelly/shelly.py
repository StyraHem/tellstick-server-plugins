...
class ShellyDevice(Device):
	...
	def _updated(self):
		if hasattr(self.dev, 'state'):
			...
			#self.setState(Device.TURNON if self.dev.state else Device.TURNOFF, onlyUpdateIfChanged=True)
		...
		if hasattr(self.dev, 'sensorValues'):
			if 'watt' in self.dev.sensorValues:
				self.setSensorValue( Device.WATT, self.dev.sensorValues['watt'], Device.SCALE_POWER_WATT )
				self.plugin.refreshClient()			
			if 'temperature' in self.dev.sensorValues:
				self.setSensorValue( Device.TEMPERATURE, self.dev.sensorValues['temperature'], Device.SCALE_TEMPERATURE_CELCIUS )
				self.plugin.refreshClient()			
...
class Shelly(Plugin):
	...
	def __init__(self):
		...
		deviceManager = self.deviceManager = DeviceManager(self.context)
	...
	def handleRequest(self, plugin, path, __params, **__kwargs):
		...
		if path == 'dump': 
			...
			devices = list(map(lambda d : { 'id' : d.id(), 'localid' : d.localId(), 'name' : d.name(), 'state' : d.state(), 'params' : d.params() }, shellyDevices ))
		...
		if path == "addMember": 
			...
			print(self.pyShelly._socket.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq))
			return WebResponseJson({})
		...
		if path == "dropMember": 
			...
			import struct
			mreq = struct.pack("=4sl", socket.inet_aton("224.0.1.187"), socket.INADDR_ANY)
			print(self.pyShelly._socket.setsockopt(socket.IPPROTO_IP, socket.IP_DROP_MEMBERSHIP, mreq))
			return WebResponseJson({})			
